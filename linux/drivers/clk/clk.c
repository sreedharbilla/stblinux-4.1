...
static void clk_core_unprepare(struct clk_core *clk)
{
	...
	if (clk->flags & CLK_IS_SW)
		for (i = 0; i < clk->num_parents; i++)
			clk_core_unprepare(clk->parents[i]);
	else
...
static int clk_core_prepare(struct clk_core *clk)
{
	...
	if (clk->prepare_count == 0) {
		if (clk->flags & CLK_IS_SW)
			for (i = 0; i < clk->num_parents; i++) {
				ret = clk_core_prepare(clk->parents[i]);
				if (ret) {
					for (j = i - 1; j >= 0; j--)
						clk_core_unprepare(
							clk->parents[j]);
...
static void clk_core_disable(struct clk_core *clk)
{
	...
	if (clk->flags & CLK_IS_SW)
		for (i = 0; i < clk->num_parents; i++)
			clk_core_disable(clk->parents[i]);
	else
	...
	if (clk->enable_count == 0) {
		if (clk->flags & CLK_IS_SW)
			for (i = 0; i < clk->num_parents && !ret; i++) {
				ret = clk_core_enable(clk->parents[i]);
				if (ret) {
					for (j = i - 1; j >= 0; j--)
						clk_core_disable(clk->parents[j]);
					break;
				}
			}
		else
...
static int __clk_init(struct device *dev, struct clk *clk_user)
{
	 ...
	 */
	if ((clk->num_parents > 1 || (clk->flags & CLK_IS_SW))
	    && !clk->parents) {
...
struct clk *clk_register(struct device *dev, struct clk_hw *hw)
{
	...
	if (!core) {
		pr_err("%s: could not allocate core\n", __func__);
		ret = -ENOMEM;
		goto fail_out;
	}
	...
	if (!core->name) {
		pr_err("%s: could not allocate core->name\n", __func__);
		ret = -ENOMEM;
		goto fail_name;
	}
	...
	if (!core->parent_names) {
		pr_err("%s: could not allocate core->parent_names\n", __func__);
		ret = -ENOMEM;
